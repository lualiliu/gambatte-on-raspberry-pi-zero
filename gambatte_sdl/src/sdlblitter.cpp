...
void SdlBlitter::setBufferDimensions(const unsigned int width, const unsigned int height)
{
  ...
  //surface = screen = SDL_CreateRGBSurface(SDL_SWSURFACE, 0, 0, 16, 0, 0, 0, 0);
  ...
  switch(scaler)
  {
    ...
    case 1:		/* Ayla's 1.5x scaler */
      if(menu_scale != scale){
        ...
        rt.w = 320;
        rt.h = 240;
        SDL_BlitSurface(menu_bk, &rt, screen, NULL);
      }
    ...
    default:
      ...
      if(menu_scale != scale){
	SDL_Rect Mdst;
     	Mdst.x = (screen->w - surface->w) / 2 - 160 + 80;
      	Mdst.y = (screen->h - surface->h) / 2 - 120 + 72;
      	Mdst.w = surface->w;
      	Mdst.h = surface->h;
        menu_scale = scale;
        rt.x = 0;
        ...
        rt.w = 320;
        rt.h = 240;
        SDL_BlitSurface(menu_bk, &rt, screen, &Mdst);
      }
      SDL_Rect dst;
...
void SdlBlitter::present()
{
  ...
  else
  {
    ...
    for(uint8_t y = 0; y < 240; y++, s += 160, d += 160)
